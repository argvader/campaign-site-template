...
description: >-
  Work out which Deepgram speaker number is which person for a {{campaign_name}}
...
Expected speakers: **{{expected_speaker_count}}** — {{dm_name}} plus the player roster.
...
Compare the count found against the **{{expected_speaker_count}}** expected:
...
```bash
jq -r --argjson names '{{character_names_json}}' '
  .results.utterances
  | group_by(.speaker)[]
  | (.[0].speaker) as $s
  | . as $utts
...
```bash
jq -r --argjson dm 0 --argjson names '{{character_names_json}}' '
...
- **Players** speak in the first person about one character ("I attack", "Fizz goes for
