# World Page Builder — {{campaign_name}}

## Task Description

This prompt builds the **home page** of the *{{campaign_name}}* site — the root
...
2. **Hero image** — generate one (see below) and embed it at the very top of the
   body, right under the `# {{campaign_name}}` H1:
   ```markdown
   ![{{campaign_name}}](assets/home-hero.png){ .home-hero }
   ```

3. **Introduction** — 2–3 short paragraphs welcoming the reader to the world and
   the campaign's starting location. Set the tone ({{genre_tone}}) drawn from
   `world/world.md`.
